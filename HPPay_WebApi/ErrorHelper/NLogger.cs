...
using System.Text;
using HPPay_WebApi.Helpers;
using Newtonsoft.Json;
using HPPay_WebApi.ErrorHelper;

namespace HPPay_WebApi.Helpers
{
    /// <summary>  
    ...
    public sealed class NLogger : ITraceWriter
    {
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Exception != null && !string.IsNullOrWhiteSpace(record.Exception.GetBaseException().Message))
            {
                ...
                {
                    if (record.Exception is ApiException exception)
                    {
                ...
                {
                    if (record.Exception is ApiBusinessException exception)
                    {
                        message.Append("").Append("Error: " + exception.ErrorDescription + Environment.NewLine);
                        message.Append("").Append("Error Code: " + exception.ErrorCode + Environment.NewLine);
                    }
                ...
                {
                    if (record.Exception is ApiDataException exception)
                    {
                        message.Append("").Append("Error: " + exception.ErrorDescription + Environment.NewLine);
                        message.Append("").Append("Error Code: " + exception.ErrorCode + Environment.NewLine);
                    }
