...
{
    public class TMFLAPIAuthenticationFilter : Attribute, IAuthorizationFilter
    {
        private readonly DapperContext _context;
        IConfiguration _configuration;        

        public TMFLAPIAuthenticationFilter(DapperContext context, IConfiguration configuration)
        {
            _context = context;
            ...
            if (actionName != "CreateCard")
            {
                ...
                if (string.IsNullOrEmpty(objObject.Username))
                {
                    context.Result = new JsonResult
                         (
                         new RouteValueDictionary(new TMFLAPIAuthenticationFailureResult("Mandatory Field is missing: Username", request, actionName).Execute()
                         ));
                }
                else if (string.IsNullOrEmpty(objObject.Password))
                {
                    context.Result = new JsonResult
                         (
                         new RouteValueDictionary(new TMFLAPIAuthenticationFailureResult("Mandatory Field is missing: Password", request, actionName).Execute()
                         ));
                }
                else if (string.IsNullOrEmpty(objObject.SecurityToken))
                {
                    context.Result = new JsonResult
                         (
                         new RouteValueDictionary(new TMFLAPIAuthenticationFailureResult("Mandatory Field is missing: Authorization", request, actionName).Execute()
                         ));
                }
                //else if (string.IsNullOrEmpty(objObject.ContentType))
                //{
                //    context.Result = new JsonResult
                //         (
                //         new RouteValueDictionary(new TMFLAPIAuthenticationFailureResult("Mandatory Field is missing: Content Type", request, actionName).Execute()
                //         ));
                //}
                else
                {
                    IEnumerable<TMFLAPIValidateCredentialsModelOutput> Validation = new List<TMFLAPIValidateCredentialsModelOutput>();
                    ...
                    if (actionName == "TMFLProcessCustomerRecharge")
                    {
                        string plainData = objTrans.CustomerId + objTrans.ControlCardNumber + objTrans.Amount + objTrans.TransactionDate +
                        objTrans.TransactionNumber + _configuration.GetSection("TMFLSettings:SecurityToken").Value;
                    ...
                    //parameters.Add("ClientCheckSum", objTrans.ClientCheckSum, DbType.String, ParameterDirection.Input);
                    ...
                    Validation = connection.Query<TMFLAPIValidateCredentialsModelOutput>(procedureName, parameters, commandType: CommandType.StoredProcedure);
                    ...
                    foreach (var obj in Validation)
                    {
                        if (obj.Status == "1")
                        {
                            context.Result = new JsonResult
                                 (
                                 new RouteValueDictionary(new TMFLAPIAuthenticationFailureResult(obj.ResponseMessage, request, actionName).Execute()
                                 ));
                        }
                        break;
                    }
                    ...
                    else
                    {
                        context.Result = new JsonResult
                                     (
                                     new RouteValueDictionary(new TMFLAPIAuthenticationFailureResult("Invalid hasKey", request, actionName).Execute()
                                     ));
                    }
                }
            }

            if (actionName == "TMFLCreateCard")
            {
                HttpContext httpContext = context.HttpContext;
                if(OutUserName.ToString()!="")
                {
                    httpContext.Session.SetString("TMFLUsername", OutUserName);
                }
                if (OutPassword.ToString() != "")
                {
                    httpContext.Session.SetString("TMFLPassword", OutPassword);
                }
                if (OutSecurityToken.ToString() != "")
                {
                    httpContext.Session.SetString("TMFLSecurityToken", OutSecurityToken.ToString().Substring(4));
                }
            }
        }
        ...
        public class TMFLAPIAuthenticationFailureResult
        {
            public string ReasonPhrase = string.Empty;
            public string MethodName = string.Empty;
            public HttpRequest Request { get; set; }

            public TMFLAPIAuthenticationFailureResult(string reasonphrase, HttpRequest request, string methodName)
            {
            ...
            public TMFLAPIReponseMessage Execute()
            {

                TMFLAPIReponseMessage response = new TMFLAPIReponseMessage
                {
                    Status = "1",
